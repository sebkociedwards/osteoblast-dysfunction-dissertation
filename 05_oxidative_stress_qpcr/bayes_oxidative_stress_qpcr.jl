...
const INPUT_FILE = joinpath(PROC_DIR, "delta_log2_fc.csv")
const OUT_FILE = joinpath(PROC_DIR, "bayes_gene_stats.csv")
...
gene_col = pick_col(df, ["gene", "target"])
...
out = DataFrame(
    ...
    p_mu_greater_1 = Float64[],
)

for g in GENE_ORDER
    sdf = filter(gene_col => ==(g), df)
...
println("\nBayesian oxidative stress qPCR summary")
